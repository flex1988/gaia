...
static void mbuf_free(struct mbuf *mbuf) {
    ...
    ga_log(DEBUG, "put mbuf %p len %d", mbuf, mbuf->last - mbuf->pos);
...
void mbuf_put(struct context *ctx, struct mbuf *mbuf) {
    ga_log(DEBUG, "put mbuf %p len %d", mbuf, mbuf->last - mbuf->pos);
...
void mbuf_insert(struct mhdr *mhdr, struct mbuf *mbuf) {
    ...
    ga_log(DEBUG, "insert mbuf %p len %d", mbuf, mbuf->last - mbuf->pos);
}
...
void mbuf_remove(struct mhdr *mhdr, struct mbuf *mbuf) {
    ga_log(DEBUG, "remove mbuf %p len %d", mbuf, mbuf->last - mbuf->pos);
 ...
 */
int mbuf_copy_from(struct context *ctx, struct mhdr *mhdr, uint8_t *pos, size_t size) {
    struct mbuf *mbuf;
    size_t n;
    ...
    STAILQ_FOREACH(mbuf, mhdr, next) { ASSERT(mbuf->magic == MBUF_MAGIC); }
...
void mbuf_copy_to(struct mhdr *mhdr, uint8_t *marker, uint8_t *pos, size_t size) {
    ...
    for (mbuf = STAILQ_FIRST(mhdr); mbuf != NULL; mbuf = STAILQ_NEXT(mbuf, next)) {
        if (mbuf_contains(mbuf, marker)) {
            n = MIN(size, mbuf->last - marker);

            ga_memcpy(pos, marker, n);
            pos += n;
            size -= n;
            break;
        }
    ...
    for (mbuf = STAILQ_NEXT(mbuf, next); mbuf != NULL && size > 0; mbuf = STAILQ_NEXT(mbuf, next)) {
        n = MIN(size, mbuf_length(mbuf));

        ga_memcpy(pos, mbuf->pos, n);
        pos += n;
        size -= n;
    }
}
 ...
 */
struct mbuf *mbuf_split(struct context *ctx, struct mhdr *h, uint8_t *pos, mbuf_copy_t cb, void *cbarg) {
    ...
    ga_log(DEBUG, "split into mbuf %p len %" PRIu32
                  " and nbuf %p len "
...
void mbuf_init(void) {
    ...
    ga_log(DEBUG, "mbuf hsize %d chunk size %zu offset %zu length %zu", MBUF_HSIZE, mbuf_chunk_size, mbuf_offset, mbuf_offset);
}
